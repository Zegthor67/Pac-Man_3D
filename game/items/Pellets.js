...
export class Pellets {
  ...
  spawnAll(pelletsPos, powerPos) {
    ...
    for (const pos of pelletsPos) {
      const mesh = new THREE.Mesh(new THREE.SphereGeometry(0.12, 8, 8), MAT_PELLET)
      mesh.position.copy(pos)
      mesh.position.y = 0.5
      this._scene.add(mesh)
      const item = { type: 'normal', mesh }
      this._items.push(item)
      this._active.add(item)
    }

    for (const pos of powerPos) {
      const mesh = new THREE.Mesh(new THREE.SphereGeometry(0.28, 10, 10), MAT_POWER)
      mesh.position.copy(pos)
      mesh.position.y = 0.5
      this._scene.add(mesh)
      const item = { type: 'power', mesh }
      this._items.push(item)
      this._active.add(item)
    }
  }
